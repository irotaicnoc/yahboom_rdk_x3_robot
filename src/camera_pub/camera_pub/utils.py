...
def format_camera_frames(frame, width: int, height: int, image_orientation: int):
    # if save_img:
    #     with open(f'/root/GIT/yahboom_rdk_x3_robot/src/camera_pub/camera_pub/{counter}_01_frame_raw.raw', 'wb') as f:
    #         f.write(frame)
    frame_from_buffer = np.frombuffer(frame, dtype=np.uint8)
    # if save_img:
    #     np.save(f'/root/GIT/yahboom_rdk_x3_robot/src/camera_pub/camera_pub/{counter}_02_frame_from_buffer.npy', frame_from_buffer)
    # logger().info(f'frame_from_buffer shape: {frame_from_buffer.shape}')
    frame_reshaped = frame_from_buffer.reshape(height * 3 // 2, width)
    # if save_img:
    #     np.save(f'/root/GIT/yahboom_rdk_x3_robot/src/camera_pub/camera_pub/{counter}_03_frame_reshaped.npy', frame_reshaped)
    ...
    # if save_img:
    #     cv2.imwrite(f'/root/GIT/yahboom_rdk_x3_robot/src/camera_pub/camera_pub/{counter}_04_frame_rgb.jpg', frame_rgb)
    ...
    # if save_img:
    #     cv2.imwrite(f'/root/GIT/yahboom_rdk_x3_robot/src/camera_pub/camera_pub/{counter}_05_flipped_frame.jpg', flipped_frame)
