...
{
    this->declare_parameter<std::float_t>("car_length",wheel_base);
    this->declare_parameter<std::float_t>("front_left_wheel_mass",mass_front_left_wheel);
    this->declare_parameter<std::float_t>("front_right_wheel_mass",mass_front_right_wheel);
    this->declare_parameter<std::float_t>("rear_left_wheel_mass",mass_rear_left_wheel);
    this->declare_parameter<std::float_t>("rear_right_wheel_mass",mass_rear_right_wheel);
    this->declare_parameter<std::float_t>("body_mass",mass_body);
    this->declare_parameter<std::float_t>("front_wheel_stiffiness",cf_);
    this->declare_parameter<std::float_t>("rear_wheel_stiffiness",cr_);
    this->declare_parameter<std::float_t>("control_period",ts_);
    this->declare_parameter<std::float_t>("max_steer_angle",max_steer_angle);
    this->declare_parameter<std::float_t>("reference_velocity",v_ref);
    this->declare_parameter<std::float_t>("horizon",horizon_);
    this->declare_parameter<std::float_t>("number_of_iteration",numOfIter);

    this->get_parameter("car_length",wheel_base); 
    this->get_parameter("front_left_wheel_mass",mass_front_left_wheel);
    this->get_parameter("front_right_wheel_mass",mass_front_right_wheel);
    this->get_parameter("rear_left_wheel_mass",mass_rear_left_wheel);
    this->get_parameter("rear_right_wheel_mass",mass_rear_right_wheel);
    this->get_parameter("body_mass",mass_body);
    this->get_parameter("front_wheel_stiffiness",cf_);
    this->get_parameter("rear_wheel_stiffiness",cr_);
    this->get_parameter("control_period",ts_);
    this->get_parameter("max_steer_angle",max_steer_angle);
    this->get_parameter("reference_velocity",v_ref);
    this->get_parameter("horizon",horizon_);
    this->get_parameter("number_of_iteration",numOfIter);

    //Init matrix
    r_ = Matrix::Identity(basic_control_size,basic_control_size);
    q_ = Matrix::Identity(basic_state_size+basic_control_size,basic_state_size+basic_control_size);
    q_ *= 20;
    state_mat_ = Matrix::Zero(basic_state_size,1);
    control_mat_ = Matrix::Zero(basic_control_size,1);
    a_mat_ = Matrix::Zero(basic_state_size,basic_state_size);
    a_mat_d_ = Matrix::Zero(basic_state_size,basic_state_size);
    b_mat_ = Matrix::Zero(basic_state_size,1);
    b_mat_d_ = Matrix::Zero(basic_state_size,1);
    c_mat_ = Matrix::Zero(basic_state_size,1);
    c_mat_d_ = Matrix::Zero(basic_state_size,1);
    ...
    lf_ = wheel_base * (1 - mass_front / mass_);
    lr_ = wheel_base * (1 - mass_rear / mass_);
    izz_ = lf_ * lf_ * mass_front + lr_ * lr_ * mass_rear;
    max_steer_angle *= M_PI/180.0;
    string pkg_name = "yahboomcar_autonomous";
...
double MpcController::mpcSolver()
{
    ...
    //预测误差空间矩阵 prediction error space matrix
    ...
    //预测状态空间矩阵 prediction state space matrix
        // A = [a b]
        //     [O i]
    ...
    // Umin - Ut <= Ai * delta_U <= Umax - Ut
    // delta_U_min <= delta_U <= delta_U_max
    
    //delta_U_min and delta_U_max 
